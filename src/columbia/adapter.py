...
class ColumbiaAdapter:
    ...
    def normalize_profile(
        ...
        row = _flatten_user(merged, full_profiles=True)
        row["source"] = self.slug
        external_id = row.get("id") or listing_payload.get("id")
        if external_id:
            row["external_id"] = str(external_id)
            row["profile_url"] = f"{self.base_url}/users/{external_id}"
        return row
